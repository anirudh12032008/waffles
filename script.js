...
function prev() {
  ...
  [...fills, ...tops].forEach(id => {
    ...
    box.style = 'margin:5px;display:flex;flex-direction:column;align-items:center;border:2px solid #ddd;border-radius:10px;padding:5px;width:100px;height:100px;';
...
function build() {
  ...
  const img = document.createElement('img');
  img.src = "https://t4.ftcdn.net/jpg/01/89/78/15/360_F_189781543_EpHDnqryinw4fBlU1L3L0YgLdYUxedfi.jpg";
  img.className = 'final';
  ...
  if (fills.length) {
    ul.innerHTML += `<li><strong>Fillings:</strong></li>` +
      fills.map(i => `<li>- ${names[i]}</li>`).join('');
  }
  if (tops.length) {
    ul.innerHTML += `<li><strong>Toppings:</strong></li>` +
...
document.querySelectorAll('.i').forEach(el => el.addEventListener('click', handleClick));
